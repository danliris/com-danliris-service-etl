...
function transform(data) {
    var result = data.dailyOperation.map((item) => {
        ...
        return {
            ...
            badOutputDescription: item.badOutputDescription ? `'${item.badOutputDescription.substring(0, 499)}'` : null,
            code: item.code ? `'${item.code.substring(0, 49)}'` : null,
            ...
            shift: item.shift ? `'${item.shift.substring(0, 49)}'` : null,
            ...
            kanbanCode: item.kanbanCode ? `'${item.kanbanCode.substring(0, 49)}'` : null,
            kanbanGrade: item.kanbanGrade ? `'${item.kanbanGrade.substring(0, 49)}'` : null,
            kanbanCartCartNumber: item.kanbanCartCartNumber ? `'${item.kanbanCartCartNumber.substring(0, 49)}'` : null,
            kanbanCartCode: item.kanbanCartCode ? `'${item.kanbanCartCode.substring(0, 49)}'` : null,
            kanbanCartPcs: item.kanbanCartPcs ? `${item.kanbanCartPcs}` : 0,
            kanbanCartQty: item.kanbanCartQty ? `${item.kanbanCartQty}` : 0,
            kanbanInstructionCode: item.kanbanInstructionCode ? `'${item.kanbanInstructionCode.substring(0, 49)}'` : null,
            kanbanInstructionName: item.kanbanInstructionName ? `'${item.kanbanInstructionName.substring(0, 49)}'` : null,
            orderType: item.orderType ? `'${item.orderType.substring(0, 24)}'` : null,
            selectedProductionOrderDetailCode: null,
            selectedProductionOrderDetailColorRequest: item.SelectedProductionOrderDetailColorRequest ? `'${item.SelectedProductionOrderDetailColorRequest.substring(0, 49).replace(/'/g, '"')}'` : null,
            selectedProductionOrderDetailColorTemplate: item.SelectedProductionOrderDetailColorTemplate ? `'${item.SelectedProductionOrderDetailColorTemplate.substring(0, 499).replace(/'/g, '"')}'` : null,
            machineCode: item.machineCode ? `'${item.machineCode.substring(0, 49)}'` : null,
            machineCondition: item.machineCondition ? `'${item.machineCondition.substring(0, 49)}'` : null,
            machineManufacture: item.machineManufacture ? `'${item.machineManufacture.substring(0, 49)}'` : null,
            machineMonthlyCapacity: item.machineMonthlyCapacity ? `${item.machineMonthlyCapacity}` : null,
            machineName: item.machineName ? `'${item.machineName.substring(0, 49)}'` : null,
            machineProcess: item.machineProcess ? `'${item.machineProcess.substring(0, 49)}'` : null,
            machineYear: item.machineYear ? `'${item.machineYear.toString().substring(0, 4)}'` : null,
            ...
            status: null,
            type: item.type ? `'${item.type.substring(0, 9)}'` : null,
            stepProcessId: item.stepProcessId ? `'${item.stepProcessId.toString().substring(0, 49)}'` : null,
            stepProcess: item.StepProcess ? `'${item.StepProcess.substring(0, 49)}'` : null,
            processArea: null,
            productionOrderNo: item.productionOrderNo ? `'${item.productionOrderNo.substring(0, 99)}'` : null,
            salesContractNo: item.salesContractNo ? `'${item.salesContractNo.substring(0, 99)}'` : null,
            action: item.action ? `'${item.action.substring(0, 254).replace(/'/g, '"')}'` : null
        }
    });
